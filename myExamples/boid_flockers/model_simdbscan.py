...
class BoidFlockers(Model, cooperativeExtension):
    ...
    def __init__(self, sim_seed = SIM_SEED, scenario=None, rng=SEED, ko_agent = KO_AGENT):
        ...
        self.history_clusters = {}  # Dizionario: {color_id: set_di_agenti}      
    ...
    def find_clusters(self):
        ...
        if db is not None:
            ...
            self.outlier_count = int(np.sum(labels == -1))

            # 1. Raggruppiamo gli ID degli agenti per i nuovi cluster
            new_clusters = {}
            for agent, label in zip(agents_list, labels):
                if label != -1:
                    if label not in new_clusters:
                        new_clusters[label] = set()
                    new_clusters[label].add(agent.unique_id)

            label_to_color = {}
            used_color_ids = set()
            new_history = {}

            # --- FASE 1: I cluster mantengono il loro vecchio colore ---
            unassigned_labels = []
            
            for label, current_agents in new_clusters.items():
                best_match_color = -1
                max_intersection = 0

                for old_color_id, old_agents in self.history_clusters.items():
                    if old_color_id in used_color_ids:
                        continue 
                    
                    intersection = len(current_agents.intersection(old_agents))
                    if intersection > max_intersection:
                        max_intersection = intersection
                        best_match_color = old_color_id

                if best_match_color != -1 and max_intersection > 0:
                    label_to_color[label] = best_match_color
                    used_color_ids.add(best_match_color)
                    new_history[best_match_color] = current_agents
                else:
                    # Lo mettiamo da parte per assegnargli un colore nuovo dopo
                    unassigned_labels.append((label, current_agents))

            # --- FASE 2: Ricicliamo i colori per i cluster nuovi ---
            for label, current_agents in unassigned_labels:
                c = 0
                # Cerchiamo il numero più piccolo (0, 1, 2...) NON attualmente usato a schermo
                while c in used_color_ids:
                    c += 1
                
                label_to_color[label] = c
                used_color_ids.add(c)
                new_history[c] = current_agents

            # 3. Aggiorniamo la memoria per lo step successivo
            self.history_clusters = new_history

            # 4. Assegniamo l'ID finale agli agenti per l'interfaccia
            for agent, label in zip(agents_list, labels):
                if label == -1:
                    agent.cluster_id = -1
                else:
                    agent.cluster_id = label_to_color[label]
