...
class BoidFlockers(Model):
    ...
    def __init__(self, scenario=None):
        ...
        """
        if scenario is None:
            scenario = BoidsScenario(rng=43)  # fresh seed every instantiation
    ...
    def step(self):
        ...
        for agent in self.agents:
            # Assumes boid has pos (tuple) and heading (tuple/array)
            features = [*agent.position, *agent.direction] 
            data.append(features)
        
        # 2. Run DBSCAN
        if len(data) > 0:
            X = np.array(data)
            # You may need to scale features if positions (e.g. 100) 
            # and directions (e.g. 1) have very different magnitudes
